...
export function HomePage() {
  ...
  return <><Seo title="TJKT SMKN 2 Lubuk Basung | Teknik Jaringan Komputer dan Telekomunikasi" description={baseDescription} />
    <section className="hero">
        ...
        <div className="hero-copy"><div className="hero-kicker"><SectionLabel dark>Program keahlian / TJKT</SectionLabel></div><h1><span className="hero-line-mask"><span className="hero-line">Menghubungkan</span></span><span className="hero-line-mask"><span className="hero-line"><em>teknologi,</em></span></span><span className="hero-line-mask"><span className="hero-line">menggerakkan</span></span><span className="hero-line-mask"><span className="hero-line">masa depan.</span></span></h1><p className="hero-sub">Tempat mengenal jaringan komputer, infrastruktur, dan telekomunikasi melalui cara berpikir teknis dan praktik yang nyata.</p><div className="hero-actions"><Link href="/pembelajaran" className="button-primary" data-testid="link-hero-explore">Jelajahi TJKT <ArrowRight size={15} /></Link><Link href="/tentang" className="button-secondary" data-testid="link-hero-about">Tentang TJKT</Link></div><div className="hero-data" aria-label="Ringkasan program"><div><strong>04</strong><span>fondasi bidang</span></div><div><strong>06</strong><span>ruang praktik</span></div><div><strong>01</strong><span>rasa ingin tahu</span></div></div></div>
          <div className="hero-visual">
           <figure className="hero-image-frame">
             ...
             <figcaption>DOKUMENTASI / PROFIL TJKT SMKN 2 LUBUK BASUNG</figcaption>
           </figure>
     ...
     <RunningBand />
      <StatementStrip />
      <section id="kompetensi" className="competency-band" aria-labelledby="competency-title"><h2 id="competency-title" className="sr-only">Kompetensi TJKT</h2><div className="container competency-grid" data-stagger-children>{competencies.map(({ number, title, description, icon: Icon }, index) => <button className={`competency ${selectedCompetency === index ? 'selected' : ''}`} key={number} onClick={() => setSelectedCompetency(index)} aria-pressed={selectedCompetency === index} data-testid={`button-competency-${number}`}><span className="competency-number">{number}</span><div className="competency-icon"><Icon size={23} strokeWidth={1.5} /></div><h3>{title}</h3><p>{description}</p><span className="competency-indicator" aria-hidden="true" /></button>)}</div></section>
     <StorySection />
...
export function LearningPage() {
  return <><Seo title="Pembelajaran TJKT | SMKN 2 Lubuk Basung" description="Area kompetensi yang relevan dengan bidang Teknik Jaringan Komputer dan Telekomunikasi." /><PageHero eyebrow="02 / Pembelajaran" title={<>Apa yang akan<br /><em>kamu pelajari?</em></>} aside="Area di bawah ini adalah contoh bidang kompetensi yang relevan dengan TJKT—bukan daftar resmi mata pelajaran sekolah." />
     <section className="section"><div className="container learning-layout"><div className="learning-sticky" data-reveal><SectionLabel>Area kompetensi</SectionLabel><h2>Belajar<br />melihat<br /><span style={{ color: 'var(--blue)' }}>hubungan.</span></h2><p>Teknologi jaringan hadir sebagai sebuah sistem. Telusuri area yang saling terhubung di sepanjang jalur ini.</p></div><div className="timeline" data-stagger-children>{learningAreas.map((area) => <article className="timeline-item" key={area.number}><span className="timeline-index">{area.number} / AREA TJKT</span><h3>{area.title}</h3><p>{area.description}</p><div className="timeline-tags">{area.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></article>)}</div></div></section>
...
export function FacilitiesPage() {
  ...
  return <><Seo title="Fasilitas & Kegiatan | TJKT SMKN 2 Lubuk Basung" description="Mengenal enam laboratorium dan contoh kegiatan praktik bidang TJKT." /><PageHero eyebrow="03 / Fasilitas & kegiatan" title={<>Ruang untuk<br /><em>mencoba.</em></>} aside="Dokumentasi ruang praktik dan kegiatan TJKT SMKN 2 Lubuk Basung, dari laboratorium komputer sampai perangkat jaringan." />
     <section className="section"><div className="container facility-hero-copy" data-reveal><SectionLabel>Laboratorium TJKT</SectionLabel><h2>Enam ruang.<br /><span>Banyak kemungkinan.</span></h2><p>Laboratorium adalah ruang untuk mengubah teori menjadi kebiasaan kerja: menyiapkan, menghubungkan, memeriksa, dan memperbaiki. Dokumentasi di bawah memperlihatkan ruang dan perangkat yang digunakan untuk belajar secara langsung.</p></div><div className="container lab-grid">{media.labs.map((lab, index) => <button type="button" className="lab-slot lab-button" style={{ '--lab-image': `url(${lab.fallback})`, '--media-position': lab.focus } as CSSProperties} key={lab.src} data-reveal onClick={() => openMedia(lab)} aria-label={`Perbesar dokumentasi ${lab.title}`} data-testid={`button-lab-${index + 1}`}><img className="slot-image" src={lab.src} alt={`Dokumentasi ${lab.title} TJKT`} loading={index < 2 ? 'eager' : 'lazy'} decoding="async" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = lab.fallback; }} /><span className="lab-meta">LAB / 0{index + 1}</span><span className="lab-expand" aria-hidden="true"><Maximize2 size={14} /></span><span className="lab-title">{lab.title}</span><small>{lab.note}</small></button>)}</div></section>
     ...
     <section className="section gallery-section"><div className="container"><div className="gallery-heading" data-reveal><div><SectionLabel>Galeri kegiatan</SectionLabel><h2 className="display">Ruang belajar<br /><span>yang nyata.</span></h2></div><p className="body-copy">Dokumentasi kegiatan dan fasilitas TJKT dari SMKN 2 Lubuk Basung.</p></div><div className="lab-grid gallery-grid">{media.gallery.map((item, index) => <button type="button" className={`lab-slot lab-button gallery-slot ${item.size}`} style={{ '--lab-image': `url(${item.fallback})`, '--media-position': item.focus } as CSSProperties} key={`${item.src}-${index}`} data-reveal onClick={() => openMedia(item)} aria-label={`Perbesar dokumentasi ${item.title}`} data-testid={`button-gallery-${index + 1}`}><img className="slot-image" src={item.src} alt={`Dokumentasi ${item.title} TJKT`} loading="lazy" decoding="async" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = item.fallback; }} /><span className="lab-meta">GALLERY / 0{index + 1}</span><span className="lab-expand" aria-hidden="true"><Maximize2 size={14} /></span><span className="lab-title">{item.title}</span><small>{item.note}</small></button>)}</div></div></section>
    <section className="container section-tight"><div className="cta-panel" data-reveal><h2>Lihat bagaimana<br /><span>TJKT terhubung.</span></h2><Link href="/kontak" className="button-primary" data-testid="link-facilities-cta">Prospek & kontak <ArrowRight size={15} /></Link></div></section>
       {selectedMedia && <div className="media-lightbox" role="dialog" aria-modal="true" aria-labelledby="media-lightbox-title" onClick={() => setSelectedMedia(null)}><div className="media-lightbox-card" onClick={(event) => event.stopPropagation()}><button ref={mediaCloseRef} type="button" className="media-lightbox-close" onClick={() => setSelectedMedia(null)} aria-label="Tutup dokumentasi" data-testid="button-close-media"><X size={19} /></button><img style={{ '--media-position': selectedMedia.focus } as CSSProperties} src={selectedMedia.src} alt={`Dokumentasi ${selectedMedia.title} TJKT`} decoding="async" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = selectedMedia.fallback; }} /><div className="media-lightbox-caption"><span className="mono">DOKUMENTASI TJKT</span><h2 id="media-lightbox-title">{selectedMedia.title}</h2><p>{selectedMedia.note}</p></div></div></div>}
   </>;
}
